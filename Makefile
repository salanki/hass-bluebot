...
test-cov: ## Run pytest with coverage report
	$(DOCKER_RUN) bash -c "pip install -q -e .[dev] pytest-cov && \
	  pytest --cov=custom_components.bluebot --cov-report=term-missing -q"
...
test-live: ## Run the opt-in read-only live tests (needs BLUEBOT_API_KEY)
	@test -n "$$BLUEBOT_API_KEY" || (echo 'BLUEBOT_API_KEY not set'; exit 1)
